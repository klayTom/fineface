...
# 你的模型权重最终保存路径
MODEL_WEIGHT_PATH = (
    "/root/autodl-tmp/fineface/fineface_spatial_id_ours/checkpoint-15000"
)
NUM_AUS = 12
...
def init_eval_models(device="cuda"):
    """初始化用于评测的基础模型"""
    print(
        "Loading CLIP Model (请确保终端已执行 export HF_ENDPOINT=https://hf-mirror.com)..."
    )
...
def get_1650_test_cases():
    ...
    for prompt in prompts:
        ...
        for i in range(50):
            ...
            test_cases.append(
                {"prompt": prompt, "target_au": au_tensor, "type": f"combo_random_{i}"}
            )
...
def load_local_fineface(model_path, device="cuda"):
    ...
    unet = UNet2DConditionModel.from_pretrained(
        sd_repo_id, subfolder="unet", torch_dtype=torch.float16
    )
    ...
    unet.load_state_dict(
        torch.load(
            os.path.join(model_path, "attn_processors.ckpt"), map_location=device
        ),
        strict=False,
    )
    unet.load_attn_procs(
        model_path, weight_name="pytorch_lora_weights.safetensors", adapter_name="unet"
    )

    pipe = StableDiffusionPipeline.from_pretrained(
        sd_repo_id, unet=unet, torch_dtype=torch.float16
    ).to(device)

    au_encoder = AUEncoder().to(device)
    au_encoder.load_state_dict(
        torch.load(os.path.join(model_path, "au_encoder.ckpt"), map_location=device)
    )
...
def main():
    ...
    for step, item in enumerate(tqdm(test_cases, desc="Evaluating")):
        ...
        target_au_dict = {
            key: target_au_tensor[0, idx].item()
            for idx, key in enumerate(PIPELINE_AU_KEYS)
            if target_au_tensor[0, idx].item() > 0
        }

        generator_neutral = torch.Generator(device=device).manual_seed(fixed_seed)
        img_neutral = pipeline(
            prompt=prompt, aus={}, generator=generator_neutral
        ).images[0]

        generator_cond = torch.Generator(device=device).manual_seed(fixed_seed)
        img_cond = pipeline(
            prompt=prompt, aus=target_au_dict, generator=generator_cond
        ).images[0]

        # ==========================================
        # 自动选择最清晰的 Spatial Mask
        # ==========================================
        ...
        if current_mask is None:
            current_mask = Image.new("L", (512, 512), 0)
            mask_info = {
                "layer_name": "none",
                "score": -1.0,
                "size": -1,
        ...
        # 计算 CLIP-I
        inputs_neutral = clip_processor(images=img_neutral, return_tensors="pt").to(
            device
        )
            ...
            try:
                ...
                if np.isnan(pred_aus).any():
                    ...
                    if step < 5 or step % 50 == 0:
                        log_dict["visual/neutral_Y0"] = wandb.Image(
                            img_neutral, caption="Neutral"
                        )
                        log_dict["visual/cond_Yau"] = wandb.Image(
                            img_cond,
                            caption=f"{item['type']}\nCLIP-I: {clip_i:.3f}, MSE: {au_mse:.3f}",
                        )
                        if current_mask is not None:
                            log_dict["visual/spatial_mask"] = wandb.Image(
                                current_mask,
                                caption=(
                                    f"Mask for Step {step} | "
                                    f"layer={mask_info['layer_name']} | "
                                    f"score={mask_info['score']:.3f} | "
                                    f"size={mask_info['size']}"
                                ),
                            )
    ...
    if single_valid > 0:
        ...
        wandb.log(
            {
                "final_single/avg_clip_i": avg_single_clip,
                "final_single/avg_au_mse": avg_single_mse,
            }
        )
        print(
            f"[单独 AU] 有效样本: {single_valid} -> CLIP-I: {avg_single_clip:.4f} | AU MSE: {avg_single_mse:.4f}"
        )
    ...
    if combo_valid > 0:
        ...
        wandb.log(
            {
                "final_combo/avg_clip_i": avg_combo_clip,
                "final_combo/avg_au_mse": avg_combo_mse,
            }
        )
        print(
            f"[组合 AU] 有效样本: {combo_valid} -> CLIP-I: {avg_combo_clip:.4f} | AU MSE: {avg_combo_mse:.4f}"
        )
    ...
    if total_valid > 0:
        ...
        wandb.log(
            {
                "final_total/avg_clip_i": avg_total_clip,
                "final_total/avg_au_mse": avg_total_mse,
            }
        )
        print(
            f"[总体平均] 有效样本: {total_valid} -> CLIP-I: {avg_total_clip:.4f} | AU MSE: {avg_total_mse:.4f}"
        )
